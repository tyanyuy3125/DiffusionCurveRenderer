...
*/
void Potrace::run(const QVector<PixelChain> &chains)
{
 ...
 */
void Potrace::findBestPath(QVector<Point> &bestPath, PixelChain chain, Eigen::MatrixXd penalties)
{
    const int nPoints = chain.length();
...
const QVector<QVector<Point>> &Potrace::polylines() const
{
    return mPolylines;
}

float Potrace::progress() const
{
    return mProgress;
}
