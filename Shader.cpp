...
Shader::Shader(ShaderType type, QObject *parent)
    : QObject(parent)
    , mType(type)
{
...
void Shader::setUniformValueArray(const QString &name, const QVector<float> &values)
{
    mProgram->setUniformValueArray(mLocations[name], values.constData(), values.size(), 1);
}

void Shader::setUniformValueArray(const QString &name, const QVector<QVector4D> &values)
{
...
void Shader::setUniformValueArray(const QString &name, const QVector<QVector2D> &values)
{
    mProgram->setUniformValueArray(mLocations[name], values.constData(), values.size());
}
